...
if __name__ == "__main__":
    ...
    for momentum, noise in zip(momenta, noise_stds):
        ...
        plt.figure(0)
        plt.plot(obj_history, label=fr'$\gamma={momentum}, \sigma={noise}$')

        plt.figure(1)
        plt.plot(np.linalg.norm(grad_history, axis=-1), label=fr'$\gamma={momentum}, \sigma={noise}$')

        plt.figure(2)
        plt.plot(np.abs(phase_history[:,0] - phase_history[:,1]), label=fr'$\gamma={momentum}, \sigma={noise}$')

        plt.figure(3)
        plt.plot(np.abs(grad_history[:,0] + grad_history[:,1]), label=fr'$\gamma={momentum}, \sigma={noise}$')
    ...
    plt.legend()
    plt.tight_layout()
    # plt.savefig("media/exp4/obj.png")
    ...
    plt.legend()
    plt.tight_layout()
    # plt.savefig("media/exp4/gradnorm.png")
    ...
    plt.legend()
    plt.tight_layout()
    # plt.savefig("media/exp4/phasediff.png")
